...
export const HomePage = () => {
  ...
  const gotoArrivalPage = useCallback(
    () =>
      navigate(`/arrival`, {
        replace: false,
      }),
  ...
  const gotoDeparturePage = useCallback(
    () =>
      navigate(`/departure`, {
        replace: false,
      }),
  ...
  const gotoRoundTripPage = useCallback(
    () =>
      navigate(`/round-trip`, {
        replace: false,
      }),
  ...
  async function onBookNowClick(e: any) {
    ...
    await reservationService.homeFormHook.handleSubmit((data) => {
      ...
      if (reservationService.state.journeyType === JourneyType.DEPARTURE) {
        ...
        reservationService.state.journeyType === JourneyType.ARRIVAL_ONE_WAY
      ) {
        gotoArrivalPage();
      } else {
        gotoRoundTripPage();
  ...
  async function onModeSelect(mode: JourneyType) {
    await TripProcessor.findPrice(
      reservationService.state.homeFormData,
      reservationService.state.journeyType
    );
  }
  ...
  function buildModeButton(mode: JourneyType, title: string, image: string) {
    ...
    return (
      <button
        ...
        aria-selected="true"
      >
        <div className="col-md nav-mode-img">
          <img className={"form-btn-svg"} src={image} alt="Ride-Mode" />
        </div>
  ...
  function buildServiceItem(params: {
    ...
    return (
      <Col lg={2} md={4} xs={6} className={"service-item"}>
  ...
  function buildPackageItem(params: {
    ...
    return (
      <Col className={"package-item-col"} xl={3} lg={4} md={6} xs={12}>
        <div className={"package-item"}>
          ...
          <div className="price-box-wrapper w-100">
            <div className="price-box">{params.price}</div>
          </div>
        </div>
      </Col>
    );
      ...
      <section className="form" id={"book-now"}>
        <div className="container py-5">
          <div className={"d-flex"}>
            <img
              src={require("../../assets/logos/ppt-logo.png")}
              alt="pdtlogo"
                  ...
                  <div className="row g-4 mt-2">
                    <div className="col-md py-3">
                      ...
                      <select
                        ...
                        {...reservationService.homeFormHook.register(
                          "pickUpPoint"
                        )}
                      >
                        <option value={"NONE"}>
                    ...
                    <div className={"text-center travel-fare-group"}>
                      <div>
                        ...
                        <span className={"price"}>
                          {" "}
                          {reservationService.homePageTripPrice}{" "}
                        </span>
                      </div>
                    </div>
                    <button
